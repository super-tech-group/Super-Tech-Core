...
public class CrudeCouplingContainer extends Container {
	private CrudeCouplingTileEntity tile;

	public CrudeCouplingContainer(IInventory playerInventory, CrudeCouplingTileEntity te) {
		this.tile = te;
	...
	private void addOwnSlots(IInventory playerInventory) {
		...
		for (int x = 0; x < CrudeCouplingTileEntity.getSlotCount(); x++) {
			addSlotToContainer(
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int sourceSlotIndex) {
		...
		if (sourceSlotIndex >= 0 && sourceSlotIndex < 36) {
			...
			if (!mergeItemStack(sourceStack, 36, 36 + CrudeCouplingTileEntity.getSlotCount(), false)) {
				return ItemStack.EMPTY; // EMPTY_ITEM
			}
		} else if (sourceSlotIndex >= 36 && sourceSlotIndex < 36 + CrudeCouplingTileEntity.getSlotCount()) {
