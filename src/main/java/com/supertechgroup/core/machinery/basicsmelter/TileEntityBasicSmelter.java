...
public class TileEntityBasicSmelter extends TileMultiBlock implements ITickable {
			...
			if (world.getTileEntity(getPos().down()) != null) {
				...
				if (te.hasCapability(HeatCapabilityProvider.HEAT_CAP, EnumFacing.DOWN)) {
					...
					if (this.inProgress == null) {
						System.out.println("Checking for start");
						tryStartSmelt();
					} else {
	...
	private void smeltItem() {
		TileEntity behind = world.getTileEntity(getPos().offset(facing));
		TileEntity behindBelow = world.getTileEntity(getPos().offset(facing).down());
		if (behind.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, facing.getOpposite())
				&& behindBelow.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, facing.getOpposite())) {
