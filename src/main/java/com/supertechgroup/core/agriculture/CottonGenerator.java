...
public class CottonGenerator implements IWorldGenerator {

	public CottonGenerator() {
	}
	...
	public void generate(Random rand, int chunkX, int chunkZ, World worldIn, IChunkGenerator chunkGenerator,
		...
		if (biome.isSnowyBiome()||!biome.isHighHumidity()) {
			return;
		}
	

		BlockBush flower = ModRegistry.cottonCrop;
		...
		for (int i = 0; i < 16; ++i) {
			...
			if (worldIn.isAirBlock(blockpos) && (!worldIn.provider.isNether() || blockpos.getY() < 255)
				...
				worldIn.setBlockState(blockpos, flower.getDefaultState().withProperty(CottonCropBlock.AGE, 7), 2);
			}
		}
